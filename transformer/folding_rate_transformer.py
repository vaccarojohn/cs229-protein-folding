...
class ProteinSequenceTransformerRegression(nn.Module):
...
def read_data(base_dir, filename):
    ...
    features = ['Sub-Sequence Used In Experiment', 'DSSP', 'ln(kf) 25', 'Length of Sub-Sequence Used In Experiment', 'B_factor', 'CO', 'Abs_CO', 'TCD', 'LR_CO']
    
    return df[normal_mask][features]
...
def main():
    ...
    x_other_data = data[['Length of Sub-Sequence Used In Experiment', 'B_factor', 'CO', 'Abs_CO', 'TCD', 'LR_CO']].values
    ...
    for i, (train_index, test_index) in enumerate(kf.split(x_text_data)):
        ...
        model = ProteinSequenceTransformerRegression()
        optimizer = torch.optim.Adam(model.parameters(), lr=0.00075, weight_decay=1e-3)
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Epoch {epoch + 1}, Loss: {total_loss / len(train_labels):.2f}")
        ...
        for seq, other, tgt in test_loader:
            output = model(seq, other)
            ax.scatter(x_other_data[:, 0][test_index], output.detach().numpy(), label='Predicted Folding Rate', color='blue', marker='o')
            ax.scatter(x_other_data[:, 0][test_index], test_labels, label='Actual Folding Rate', color='green', marker='x')
            loss = criterion(output, tgt.to(torch.float32))
            total_loss += loss.item()
        ...
        print(f"Loss: {total_loss / len(test_labels):.2f}")
        final_transformer_loss += total_loss

    print(f"Final Loss: {final_transformer_loss / (5 * len(test_labels)):.2f}")
